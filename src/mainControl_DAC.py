...
def brake_cb(data):
    ...
    accelerate(10)  # TODO: Test for right sequance to avoid stopping
...
def callback_speed_feedback(data): #add odommsg as global
    ...
    # Car not moving and cmd not zero and in planning since 10 sec.
        ...
        zeroSpeedCounter = 0
    #if zeroSpeedCounter == 45:
    #    print ('>>>>>>>>>>>>>>>>>>>>>> Apply SWITCH')
    if zeroSpeedCounter >= 10: #was 10
        # HEMA 2020-06-13
        accelerate(10) # 0.52 v
        time.sleep(.2) # wait until car controller read the switch and pedal value
        #accelerate(9)
        #time.sleep(0.2)
        #New Restart 2020-06-14
        '''
        GPIO.output(accSwitch, 1)
        time.sleep(1)
        GPIO.output(accSwitch, 0)
